...
@protocol WXWPlusButtonSubclassing
 ...
 * @attention 以下两种情况下，必须实现该协议方法，否则 WXWTabBarController 会抛出 exception 来进行提示：
...
+ (CGFloat)multiplerInCenterY WXW_DEPRECATED("Deprecated in 1.6.0. Use `+multiplierOfTabBarHeight:` instead.");
...
FOUNDATION_EXTERN UIButton<WXWPlusButtonSubclassing> *WXWExternPlusButton;
FOUNDATION_EXTERN UIViewController *WXWPlusChildViewController;

@interface WXWPlusButton : UIButton
...
@interface WXWPlusButton (WXWDeprecated)

+ (void)registerSubclass WXW_DEPRECATED("Deprecated in 1.6.0. Use `+[WXWPlusButton registerPlusButton]` instead.");
